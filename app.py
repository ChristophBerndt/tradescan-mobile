...
try:
    ...
    raise SystemExit("pip install ta pandas flask gunicorn requests")
...
def analyze_df(stooq_sym, name, market, df):
    try:
        ...
        if not cond_trend:
            missing.append(f"Preis ueber MA200 ({round(ma200_now,2)})")
...
<div id="vHome">
  <div class="hd">
    ...
    <div class="btns">
      <form method="post" action="/scan" style="margin:0">
        <button class="b1" type="submit">&#9654; Vollscan ({{ total_tickers }})</button>
      </form>
      <form method="post" action="/scan-wl" style="margin:0">
        <button class="b2" type="submit">&#9711; Watchlist ({{ wl_count }})</button>
      </form>
    </div>
  </div>

  {% if scanning %}
  <div class="scan-banner">&#9889; Scan laeuft... ca. 30 Sek. Seite aktualisiert automatisch.</div>
  {% endif %}

  <div class="srch">
    <div class="srch-box">
      <span style="color:#ccc;font-size:14px">&#9906;</span>
      <input type="text" id="si" placeholder="Suchen... z.B. AAL, Japan, Vodafone" oninput="doSearch(this.value)" autocomplete="off">
    </div>
  ...
  <div class="empty">
    ...
    <div class="empty-s">Tippe auf Watchlist fuer einen schnellen Scan<br>(ca. 30 Sekunden fuer 15 Aktien)<br><br>Vollscan: alle 110 Aktien ca. 2 Minuten</div>
  </div>
  {% else %}
  <div id="homeList">
    {% for sig, sig_label, sig_dot in [('KAUFEN','Jetzt kaufen','#00A65A'),('FAST','Morgen pruefen','#FF6200'),('VERKAUFEN','Jetzt verkaufen','#E8333C')] %}
    {% set group = data | selectattr('signal','equalto',sig) | list %}
    {% if group %}
    <div class="sh">
      <span class="sh-t">{{ sig_label }}</span>
      <span class="sh-a" onclick="openAll('{{ sig }}')">Alle {{ group|length }} &rarr;</span>
    </div>
    {% for r in group[:3] %}
    <div class="row" onclick="openDetail('{{ r.ticker }}')">
      <div class="row-l">
        ...
        <div>
          ...
          <div class="nm">{{ r.name }} &middot; {{ r.market }}</div>
        </div>
      </div>
      <div class="row-r">
        <div>
  ...
  {% if data %}
  <div class="va-btn" onclick="openAll('ALLE')">Alle {{ total }} Aktien anzeigen &rarr;</div>
  {% endif %}
</div>
...
<div id="vDetail" style="display:none">
  <div class="nav">
    <button class="nav-back" id="detBack" onclick="show('vHome')">&#8249; Zurueck</button>
    <div>
...
function rowHTML(s){
  ...
  return `<div class="row" onclick="openDetail('${s.ticker}')">
    ...
    <div class="row-r">
    <div><div class="pr">${s.price_fmt}</div><div class="ch" style="color:${cc}">${(s.change>=0?'+':'')+s.change}%</div></div>
...
function openAll(filter){
  ...
  if(!html) html='<div class="empty"><div class="empty-i">&#128269;</div><div class="empty-t">Keine Daten</div><div class="empty-s">Scan starten um Daten zu laden.</div></div>';
...
function filterRegion(r){
  ...
  if(!html) html='<div style="text-align:center;padding:24px;color:#aaa;font-size:13px">Keine Signale fuer diese Region</div>';
...
function openDetail(ticker){
  ...
  if(s.signal==='KAUFEN'){
    action=`<div class="det-sec"><div class="dst">Trade-Details</div>
      ...
      <div class="det-sec"><div class="dst">Technische Werte</div>
    ...
    action=`<div class="det-sec"><div class="dst">Zukuenftiger Einstieg</div>
      ...
      <div style="margin-top:10px"><div style="font-size:10px;font-weight:700;color:#aaa;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px">Was fehlt noch?</div>${miss}</div></div>`;
    ...
    action=`<div class="det-sec"><div class="dst">Verkauf-Signal</div>
      ...
      <div class="sb-row"><span class="sl">Empfehlung</span><span class="sv">Position schliessen!</span></div></div>
      <div class="hint" style="color:#E8333C;margin-top:8px">&#9888; Ziel erreicht - Gewinn jetzt sichern!</div></div>`;
  } else {
    const miss=s.missing.map(m=>`<div class="hint">&rarr; ${m}</div>`).join('');
    action=`<div class="det-sec"><div class="dst">Zukuenftiger Einstieg</div>
      <div class="fb">
      ...
      <div class="fr"><span class="fl">Ziel 1 - MA20</span><span class="fv" style="color:#00A65A">${s.target1}</span></div></div>
...
function doSearch(v){
  ...
  r.innerHTML=m.map(s=>{const c=SIG[s.signal]||SIG.NEUTRAL;
    ...
    <div><div style="font-size:13px;font-weight:700;color:#1a1a1a">${s.ticker}</div>
    <div style="font-size:11px;color:#aaa">${s.name} - ${s.market}</div></div>
...
def index():
    ...
    return render_template_string(TEMPLATE,
        ...
        kaufen=kaufen, fast=fast, verkaufen=verkaufen, total=len(data),
        total_tickers=len(TICKERS), wl_count=len(WATCHLIST),
